...
// TODO вынести определение функции (второго параметра)
// TODO селекторы по узлам ast
// TODO внешний вид тестов
...
module.exports = {
    meta: {
        ...
        messages: {
            avoidLodashMap: "Avoid using lodash map"
        },
    },
    ...
    create: function(context) {
        ...
        // Проверка на вызов map у _
        function isLodashMapCall(node) {
            const object = node.object.name;
            const func = node.property.name;

            return object === LODASH && func === MAP;
        }
        ...
        function fix(node) {
            ...
            const iterableText = getText(iterable);
            const nativeText = `${iterableText}.map(${getText(func)})`;

            // Далее параметр name указан для вызова из iife
            // Генерация вызова _.map
            function getInitialText(name) {
                return `_.map(${name || iterableText}, ${getText(func)})`;
            }
            // Генерация нативного вызова
            function getNativeText(name) {
                return `${name || iterableText}.map(${getText(func)})`;
            }

            function getTextWithCheck(name) {
                return `${getCheckText(node, name)} ? ${getNativeText(name)} : ${getInitialText(name)}`;
            }
            ...
            if (isCall(iterable)) {
                ...
                const iifeStart = `(function (${parameterName}) {\n${indent.repeat(size + 1)}`;
                const iifeEnd = `\n${indent.repeat(size)}})(${iterableText})`;
                const body =getTextWithCheck(parameterName);
        ...
        return {
            ...
            MemberExpression: function(node) {
                ...
                // Не похоже на _.map, если передано более двух аргументов
                if (args.length > 2) {
                    return;
                }
                const [iterable, callback] = args;

                if (isLodashMapCall(node) && !isObject(iterable) && !isLiteral(callback) && !hasCheck(node)) {
                    context.report({
                        node,
                        message: "Replace lodash map with native map",
                        fix: function(fixer) {
